...
void main() {
    ...
    float ratio_glass = 1.0f / 3.52f;

    vec3 refraction_dir = refract(look_dir, normalize(fs_in.normal_v), ratio_glass);

    frag_color = vec4(texture(sampler_skybox, refraction_dir).rgb, 1.0f);
