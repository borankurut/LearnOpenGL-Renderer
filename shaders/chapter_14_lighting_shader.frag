...
void main() {
    ...
    // diffuse
    vec3 to_light_dir = normalize(light.world_position - fragment_world_position);

    float diffuse_component = max(dot(to_light_dir, normal), 0.0f);
    vec3 diffuse_color = diffuse_component * light.diffuse * material.diffuse;
    ...
    vec3 specular_color = specular_component * light.specular * material.specular;
