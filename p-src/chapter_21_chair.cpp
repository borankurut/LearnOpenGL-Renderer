...
int main()
{
    ...
    Model chair_object = model_create("models/chair/chair.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model_chair = glm::translate(model_chair, glm::vec3(0.0f, 0.0f, -3.0f));

        shader_set_mat4(lighting_shader, "model", model_chair);

        glm::mat3 normal_matrix = glm::mat3(glm::transpose(glm::inverse(model_chair)));
        shader_set_mat3(lighting_shader, "normal_matrix", normal_matrix);

        shader_set_float(lighting_shader, "material.shininess", 32.0f);
        model_draw(chair_object, lighting_shader);
